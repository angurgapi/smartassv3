    ...
    createBrain() {
      ...
      class Brain {
        constructor(savedBrain) {
          ...
          if (savedBrain) {
            // Load saved neurons
            for (const loadNeuron of savedBrain.neurons) {
            ...
            // Load saved synapses
            for (const loadSynapse of savedBrain.synapses) {
              const from = this.neurons[loadSynapse.from]
              const to = this.neurons[loadSynapse.to]
        ...
        createNeuron() {
          const neuron = new Neuron(this, ...arguments)
      ...
      class Neuron {
        ...
        createSynapse(neuron) {
          const synapse = new Synapse(this.brain, this, neuron)
        ...
        animate(progress) {
          const difference = this.baseSize - this.size
      ...
      class Synapse {
        ...
        createElectron() {
          const electron = new Electron(this.brain, this)
          this.electrons.push(electron)
      ...
      class Electron {
        ...
        getPosition() {
          // let x, y
          const progress = this.reverseDirection
            ? easeInOutQuad(this.progress)
            : easeInOutQuad(1 - this.progress)
          const toOffset = this.synapse.to.baseSize - this.size * 1.5
          const fromOffset = this.synapse.to.baseSize - this.size * 1.5
          const direction = this.synapse.getDirection()
        ...
        animate(progress) {
          ...
          if (this.progress > 1) {
            const toSynpses = this.getTo().synapses
            const randomSynapse =
              toSynpses[Math.floor(Math.random() * toSynpses.length)]

            const oldTo = this.getTo()
            this.progress = 0
      ...
      function anim(timestamp) {
        ...
        for (const electron of brain.electrons) {
      ...
      function draw() {
        ...
        for (const synapse of brain.synapses) {
          ...
          const width = 12
          const widthMiddle = 2
          const middle = between(synapse.from, synapse.to, 0.5)
          const point1 = between(synapse.from, synapse.to, 0.25)
          const point2 = between(
            synapse.from,
            synapse.to,
            0.06 + (synapse.from.size - synapse.from.baseSize) * 0.06
          )
          const point3 = between(
            synapse.from,
            synapse.to,
            0.94 - (synapse.to.size - synapse.to.baseSize) * 0.06
          )
          const point4 = between(synapse.from, synapse.to, 0.75)
          const perp = {
            // Perpendicular
        ...
        for (const electron of brain.electrons) {
          ...
          const direction = electron.getDirection()
          const fontSide = {
      ...
      const brain = new Brain(savedBrain)
      window.requestAnimationFrame(anim)
    },
  },
}
